...
export class VideoProcessor {
  static async extractMetadata(filePath: string): Promise<VideoMetadata | null> {
    return new Promise((resolve) => {
      ...
      ffprobe.on('close', (code) => {
        if (code !== 0) {
          console.warn('ffprobe failed, falling back to basic metadata');
          resolve(null);
          return;
        }
        ...
        try {
          ...
          resolve({
            ...
            size: stats.size,
            mimeType: this.getMimeTypeFromCodec(videoStream.codec_name)
          });
        } catch (error) {
